...
try:
    ...
    raise ImportError("Install requirements-deep.txt to check Phase 3") from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify frozen Phase 3 checkpoint")
    ...
    output_path = args.checkpoint.parent / "phase3_check.json"
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump({"status": "passed", "checks": checks}, handle, indent=2)
        handle.write("\n")
    print("All Phase 3 checkpoint checks passed")
